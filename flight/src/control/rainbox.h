...
void airtrack2(Mat &sourceImage, Mat &fake, int xx, int yy, int xxd, int yyd, string lr);
void scape2(Mat &fake, int xx, int yy, int &xxd, int &yyd, string lr);

void rgbcode(int &r, int &g, int &b, int num);
void colorcode(int r, int g, int b, int &num);
int pixValue(int x, int y,  Mat &sourceImage);

void ensancha(Mat &sourceImage, Mat &fake, int largo);
...
void ensancha(Mat &sourceImage, Mat &fake, int largo)
{
    ...
    int ysize = sourceImage.rows-100;
    int xsize = sourceImage.cols-100;

    for(int c=0;c<largo;c++)
    {
        for(int y=100; y<ysize; y++)
        {
            for(int x=100;x<xsize;x++)
            {
                p = (uchar*) sourceImage.ptr<uchar>(y);
                int r = p[x * channels + 2];
                int g = p[x * channels + 1];
                int b = p[x * channels + 0];   
                if(g==255 && r<100 && b<100)
                {
                    p = (uchar*) fake.ptr<uchar>(y-1);
                    p[x * channels + 2] = r;
                    p[x * channels + 1] = g;
                    p[x * channels + 0] = b;                 
                    p = (uchar*) fake.ptr<uchar>(y+1);
...
void airtrack2(Mat &sourceImage, Mat &fake, int xx, int yy, int xxd, int yyd, string lr)
{
    ...
    {
        int midx=xxd/2;
        scape2(fake, midx, yyd,xxd, yyd, lr);

        trayectory(sourceImage, fake, xx, yy, xxd, yyd); 
    }
    if(lr == "DERECHA")
    {
        int xsize = sourceImage.cols;
...
void scape2(Mat &fake, int xx, int yy, int &xxd, int &yyd, string lr)
{
    int channels = fake.channels();
    uchar* p;
    int increment = -1;
    if(lr == "DERECHA")
        increment =1;
    int stopm=0;
    while(xx!=xxd && stopm==0)
    {
        uchar* p;
        p = (uchar*) fake.ptr<uchar>(yyd);
        int r = p[(xxd+(15*increment)) * channels + 2];
        int g = p[(xxd+(15*increment)) * channels + 1];
        int b = p[(xxd+(15*increment)) * channels + 0];  
